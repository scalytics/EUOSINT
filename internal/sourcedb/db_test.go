...
func TestZoneBriefingsCacheRoundTripAndStale(t *testing.T) {
	dir := t.TempDir()
	dbPath := filepath.Join(dir, "sources.db")
	db, err := Open(dbPath)
	if err != nil {
	...
	briefings := []model.ZoneBriefingRecord{
		{LensID: "ukraine", Title: "Ukraine", Source: "UCDP GED"},
		{LensID: "sudan", Title: "Sudan", Source: "UCDP GED"},
	}
	if err := db.UpsertZoneBriefingsCache(context.Background(), briefings, now, 24*time.Hour); err != nil {
		t.Fatal(err)
	}

	loadedFresh, err := db.LoadZoneBriefingsCache(context.Background(), now.Add(2*time.Hour))
	if err != nil {
		t.Fatal(err)
	}
	if !loadedFresh.Found {
		t.Fatal("expected cache rows to exist")
	}
	if loadedFresh.Stale {
		t.Fatal("expected cache to be fresh")
	}
	if len(loadedFresh.Briefings) != 2 {
		t.Fatalf("expected 2 briefings, got %d", len(loadedFresh.Briefings))
	}

	loadedStale, err := db.LoadZoneBriefingsCache(context.Background(), now.Add(25*time.Hour))
	if err != nil {
		t.Fatal(err)
	}
	if !loadedStale.Stale {
		t.Fatal("expected cache to be stale after ttl")
	}
}

func TestZoneBriefingsCacheTrimmedOnUpsert(t *testing.T) {
	dir := t.TempDir()
	dbPath := filepath.Join(dir, "sources.db")
	db, err := Open(dbPath)
	if err != nil {
	...
	now := time.Date(2026, 3, 20, 9, 0, 0, 0, time.UTC)
	if err := db.UpsertZoneBriefingsCache(context.Background(), []model.ZoneBriefingRecord{
		{LensID: "ukraine", Title: "Ukraine"},
		{LensID: "sudan", Title: "Sudan"},
	}, now, 7*24*time.Hour); err != nil {
		t.Fatal(err)
	}
	if err := db.UpsertZoneBriefingsCache(context.Background(), []model.ZoneBriefingRecord{
		{LensID: "ukraine", Title: "Ukraine"},
	}, now.Add(2*time.Hour), 7*24*time.Hour); err != nil {
		t.Fatal(err)
	}

	loaded, err := db.LoadZoneBriefingsCache(context.Background(), now.Add(3*time.Hour))
	if err != nil {
		t.Fatal(err)
	}
	if len(loaded.Briefings) != 1 {
		t.Fatalf("expected trimmed cache to 1 record, got %d", len(loaded.Briefings))
	}
	if loaded.Briefings[0].LensID != "ukraine" {
		t.Fatalf("expected remaining lens ukraine, got %q", loaded.Briefings[0].LensID)
	}
}
