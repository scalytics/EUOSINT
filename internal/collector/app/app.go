...
func Run(ctx context.Context, args []string, stdout io.Writer, stderr io.Writer) error {
	...
	fs.StringVar(&cfg.BrowserWSURL, "browser-ws-url", cfg.BrowserWSURL, "Remote browser WebSocket URL (CDP), e.g. ws://browser:3000")
	fs.IntVar(&cfg.BrowserMaxConcurrency, "browser-max-concurrency", cfg.BrowserMaxConcurrency, "Maximum concurrent browser tab sessions")
	fs.IntVar(&cfg.BrowserConnectRetries, "browser-connect-retries", cfg.BrowserConnectRetries, "Remote browser connect attempts before local fallback")
	fs.IntVar(&cfg.BrowserConnectRetryDelayMS, "browser-connect-retry-delay-ms", cfg.BrowserConnectRetryDelayMS, "Delay between remote browser connect attempts")
	...
	fs.BoolVar(&cfg.DDGSearchEnabled, "ddg-search-enabled", cfg.DDGSearchEnabled, "Enable DDG-based candidate URL discovery")
	fs.IntVar(&cfg.DDGSearchMaxQueries, "ddg-search-max-queries", cfg.DDGSearchMaxQueries, "Maximum DDG search queries per discovery run")
	fs.IntVar(&cfg.DDGSearchDelayMS, "ddg-search-delay-ms", cfg.DDGSearchDelayMS, "Delay in milliseconds between DDG search queries")
	fs.BoolVar(&cfg.DiscoverSocialEnabled, "discover-social-enabled", cfg.DiscoverSocialEnabled, "Enable social source discovery (X/Telegram) for conflict/piracy/terror categories")
	fs.IntVar(&cfg.DiscoverSocialMaxTargets, "discover-social-max-targets", cfg.DiscoverSocialMaxTargets, "Maximum social discovery targets per run")
