...
var supportedLenses = []lensDef{
	{ID: "gaza", Title: "Gaza", OverlayType: "conflict", CoverageNote: "Structured conflict context from UCDP GED; use live feeds for breaking updates.", CountryCodes: makeSet("PS", "IL", "EG", "LB", "JO"), Bounds: bounds{29.5, 32.0, 34.8, 36.5}},
	{ID: "sudan", Title: "Sudan", OverlayType: "conflict", CoverageNote: "Structured conflict context from UCDP GED; use live feeds for breaking updates.", CountryCodes: makeSet("SD", "SS", "TD", "CF", "ET", "ER"), Bounds: bounds{3.0, 21.5, 23.5, 39.5}},
	{ID: "ukraine", Title: "Ukraine South", OverlayType: "conflict", CoverageNote: "Structured conflict context from UCDP GED; use live feeds for breaking updates.", CountryCodes: makeSet("UA", "RU", "RO", "BG", "TR"), Bounds: bounds{43.0, 27.0, 49.5, 39.5}},
	{ID: "red-sea", Title: "Red Sea", OverlayType: "maritime", CoverageNote: "Structured conflict context from UCDP GED; maritime live feeds remain primary for immediate route risk.", CountryCodes: makeSet("YE", "SA", "EG", "SD", "ER", "DJ", "SO"), Bounds: bounds{10.0, 31.0, 31.8, 45.5}},
	{ID: "sahel", Title: "Sahel", OverlayType: "terror", CoverageNote: "Structured conflict context from UCDP GED; use live feeds for breaking updates.", CountryCodes: makeSet("ML", "NE", "BF", "MR", "DZ", "TD"), Bounds: bounds{10.0, -17.5, 24.5, 25.0}},
	{ID: "drc-east", Title: "DRC East", OverlayType: "conflict", CoverageNote: "Structured conflict context from UCDP GED; use live feeds for breaking updates.", CountryCodes: makeSet("CD", "RW", "UG", "BI"), Bounds: bounds{-8.5, 27.0, 4.5, 31.8}},
}
...
func buildLensBrief(lens lensDef, items []parse.UCDPItem, now time.Time) model.ZoneBriefingRecord {
	...
	for _, item := range items {
		...
		if strings.TrimSpace(item.CountryCode) != "" {
			countryIDCounts[strings.TrimSpace(item.CountryCode)]++
			if strings.TrimSpace(item.Country) != "" {
				countryLabelByID[strings.TrimSpace(item.CountryCode)] = strings.TrimSpace(item.Country)
			}
		}
	...
	topCountryIDs := topKeys(countryIDCounts, 4)
	...
	sourceURL := ""
	if len(topCountryIDs) > 0 {
		sourceURL = "https://ucdp.uu.se/country/" + topCountryIDs[0]
	}
