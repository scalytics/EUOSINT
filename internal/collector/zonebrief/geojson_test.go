...
func TestBuildConflictZonesGeoJSONFromBoundariesFallsBackToRectangleForZoneLens(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "countries.geojson")
	body := `{
  "type": "FeatureCollection",
  "features": [
    {
      ...
      "geometry": {"type": "Polygon", "coordinates": [[[34,31],[34,33],[35,33],[35,31],[34,31]]]}
    }
  ]
}`
	if err := os.WriteFile(path, []byte(body), 0o644); err != nil {
	...
	if len(parsed.Features) == 0 {
		t.Fatal("expected fallback rectangle feature")
	}
	if parsed.Features[0].Geometry.Type != "Polygon" {
		t.Fatalf("expected polygon geometry, got %q", parsed.Features[0].Geometry.Type)
	}
